...
def criterion(actual,pred):
    ...
    dev_ac=tactual-torch.mean(tactual,dim=1,keepdim=True)
    dev_pr=tpred-torch.mean(tpred,dim=1,keepdim=True)
    dev_ac=dev_ac-torch.mean(dev_ac,dim=1,keepdim=True)
    dev_pr=dev_pr-torch.mean(dev_pr,dim=1,keepdim=True)
    fenzi=torch.sum(dev_ac.mul(dev_pr),dim=1,keepdim=True)
    fenmu1=torch.rsqrt(torch.sum(dev_ac.mul(dev_ac),dim=1,keepdim=True))
    fenmu2=torch.rsqrt(torch.sum(dev_pr.mul(dev_pr),dim=1,keepdim=True))
    cor=fenzi.mul(fenmu1.mul(fenmu2))
    lossvalue=torch.mean(1-cor)
    return lossvalue
...
rec=open("/mnt/Storage/home/tuser/hanya/CRISPR_shRNA_TPM_intersection_data_prediect_essential/after_express_across_cellline_essential_within_cellline/nor_loss_pearson/without_nor_crispr_loss_pearson_cell_gene_kerner_12_7.txt","w")
...
for i in range(3000):   
    ...
    if (i+1) % 5 == 0:
        ...
        torch.save(model4,"/mnt/Storage/home/tuser/hanya/CRISPR_shRNA_TPM_intersection_data_prediect_essential/after_express_across_cellline_essential_within_cellline/nor_loss_pearson/loss_pearson_kerner_12_stride_7_epoch"+str(i))
