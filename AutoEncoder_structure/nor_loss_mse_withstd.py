...
def criterion(actual,pred):
    ...
    tpred=(tpred-(torch.mean(tpred,dim=1,keepdim=True)))/torch.std(tpred,dim=1,keepdim=True)
    tactual=(tactual-(torch.mean(tactual,dim=1,keepdim=True)))/torch.std(tactual,dim=1,keepdim=True)
...
rec=open("/mnt/Storage/home/tuser/hanya/CRISPR_shRNA_TPM_intersection_data_prediect_essential/after_express_across_cellline_essential_within_cellline/nor_loss_mse_withstd/nor_crispr_loss_mse_with_std_kerner_12_7.txt","w")
...
for i in range(3000):   
    ...
    if (i+1) % 5 == 0:
        ...
        torch.save(model4,"/mnt/Storage/home/tuser/hanya/CRISPR_shRNA_TPM_intersection_data_prediect_essential/after_express_across_cellline_essential_within_cellline/nor_loss_mse_withstd/loss_mse_with_std_kerner_12_stride_7_epoch"+str(i))
