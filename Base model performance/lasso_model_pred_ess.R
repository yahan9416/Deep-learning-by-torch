...
one_gene_model<-function(gene){
  ...
  data=as.matrix(data)
  dtest=as.matrix(dtest)
  mod <- cv.glmnet(data, essentiality)
  y_pre=predict.cv.glmnet(mod,dtest)
  test_model_pre<<-cbind(test_model_pre,y_pre)
  test_model_pre_cor<<-rbind(test_model_pre_cor,c(gene,unlist(cor.test(y_pre,co_exp_ess[index[386:551],36]))[c(3,4)]))
  model<<-c(model,gene<-list(mod))
}
result=apply(matrix(gene_order),1,one_gene_model)

saveRDS(test_model_pre,file="Lasso_model_cor_net_nest_pca_test_predict.rds")
saveRDS(test_model_pre_cor,file="Lasso_model_cor_net_nest_pca_test_predict_gene_cor.rds")
saveRDS(model,file="Lasso_model_cor_net_nest_pca.rds")
